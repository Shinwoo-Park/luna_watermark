...
# Optional dependency: only for type hint. Fallback to 'object' if missing.
try:
    from visualize.data_for_visualization import DataForVisualization  # type: ignore
except Exception:  # pragma: no cover
    class DataForVisualization(object):  # minimal stub
        pass
...
class BaseConfig:
    ...
    def __init__(self, algorithm_config_path: str, transformers_config: TransformersConfig, *args, **kwargs) -> None:
        ...
        """
        # NOTE: algorithm_name is a @property; do NOT call it as a function.
        self.config_dict = (
            load_config_file(f'config/{self.algorithm_name}.json')
            if algorithm_config_path is None else
            load_config_file(algorithm_config_path)
        )
